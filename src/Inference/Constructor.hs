  ...
  case (HashSet.toList candidates, mExpectedTypeName) of
    ([], Just expectedTypeName) ->
      err "Undefined constructor"
        [ annotate (colorDull Green) (pretty expectedTypeName)
    ...
    (xs, _) -> err "Ambiguous constructor"
      ...
      , "Possible candidates:"
      PP.<+> prettyHumanList "and" (PP.annotate (colorDull Green) . pretty <$> xs)
      <> "."
      ]
  where
    ...
    constrDoc = case HashSet.toList cs of
      (QConstr _ cname:_) -> annotate (color Red) (pretty cname)
