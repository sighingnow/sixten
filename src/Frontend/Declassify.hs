...
throwInvalidInstance
  ...
  , annotate bold "Expected:" PP.<+> "an instance of the form" PP.<+> annotate (colorDull Green) "instance ... => C as where ..." <> ", where" PP.<+> annotate (colorDull Green) "C" PP.<+> "is a class."
  ]
...
throwMethodProblem className missingMethods extraMethods duplicates
  ...
  $ "Invalid instance"
  : (concat $
    [ if null missingMethods then [] else ["The instance is missing an implementation for:" PP.<+> prettyHumanList "and" (annotate (color Red) . pretty <$> missingMethods) <> "."]
    , if null extraMethods then [] else ["The" PP.<+> annotate (colorDull Green) (pretty className) PP.<+> "class does not define:" PP.<+> prettyHumanList "and" (annotate (color Red) . pretty <$> extraMethods) <> "."]
    , if null duplicates then [] else ["Duplicate implementations for:" PP.<+> prettyHumanList "and" (annotate (color Red) . pretty <$> duplicates) <> "."]
    ])
